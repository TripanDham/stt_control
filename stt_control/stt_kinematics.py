...
class DrivePublisherNode(hm.HelloNode):
    def __init__(self):
        ...
        self.odom_subscriber = self.create_subscription(Odometry, '/a/odom', self.odom_callback, 1, callback_group=self.callback_group)

        self.joint_states_subscriber = self.create_subscription(JointState, '/a/stretch/joint_states', qos_profile=1, callback=self.joint_states_callback, callback_group=self.callback_group)

        self.base_pub = self.create_publisher(Twist, '/a/stretch/cmd_vel', 10, callback_group=self.callback_group)
